...
public class P_53_42861 {
    ...
    public int book_solution(int n, int[][] costs){
        // 비용 순서대로 간선을 정렬해주기
        Edge[] edges = Arrays.stream(costs).map(edge -> new Edge(edge[0], edge[1], edge[2])).sorted(Comparator.comparingInt(e -> e.cost)).toArray(Edge[]::new);
